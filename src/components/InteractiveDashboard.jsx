...
export default function InteractiveDashboard() {
  ...
  // Layer 0: Main Grid (Fades, blurs, scales down, glow fades)
  const layer0Scale = useTransform(scrollYProgress, [0, 0.2], [1, 0.92]);
  const layer0BlurOrigin = useTransform(scrollYProgress, [0, 0.2], [0, 10]);
  const layer0Blur = useTransform(layer0BlurOrigin, v => `blur(${v}px)`);
  // Faster glow dropoff
  const layer0Glow = useTransform(scrollYProgress, [0, 0.1], [1, 0]);

  // Layer 1: Carbon Credits Card
  const layer1Y = useTransform(scrollYProgress, [0, 0.3], ["100vh", "15vh"]);
  const layer1Scale = useTransform(scrollYProgress, [0.3, 0.6], [1, 0.94]);
  const layer1BlurOrigin = useTransform(scrollYProgress, [0.3, 0.6], [0, 8]);
  const layer1Blur = useTransform(layer1BlurOrigin, v => `blur(${v}px)`);
  // Glow hits 1 exactly when Layer 1 is highest (between 0.25 and 0.4), fades out totally by 0.6
  const layer1Glow = useTransform(scrollYProgress, [0.1, 0.25, 0.4, 0.6], [0, 1, 1, 0]);

  // Layer 2: Filter Status Card
  // Starts coming up at 0.15 BEFORE Layer 1 finishes docking! (Cascading overlap)
  const layer2Y = useTransform(scrollYProgress, [0, 0.15, 0.6], ["100vh", "100vh", "25vh"]);
  const layer2Scale = useTransform(scrollYProgress, [0.6, 0.85], [1, 0.96]);
  const layer2BlurOrigin = useTransform(scrollYProgress, [0.6, 0.85], [0, 6]);
  const layer2Blur = useTransform(layer2BlurOrigin, v => `blur(${v}px)`);
  // Glow hits 1 exactly when Layer 2 is highest (between 0.55 and 0.7), fades out totally by 0.85
  const layer2Glow = useTransform(scrollYProgress, [0.4, 0.55, 0.7, 0.85], [0, 1, 1, 0]);

  // Layer 3: Energy Impact Card
  // Starts coming up at 0.45 BEFORE Layer 2 finishes docking! (Cascading overlap)
  const layer3Y = useTransform(scrollYProgress, [0, 0.45, 0.85], ["100vh", "100vh", "35vh"]);
  // Last layer keeps its glow when fully scrolled up
  const layer3Glow = useTransform(scrollYProgress, [0.7, 0.85], [0, 1]);

  return (
    <div ref={containerRef} className="w-full h-[350vh] relative font-[family-name:var(--font-geist)] font-sans mt-24">
      
      {/* Sticky Checkpoint Container */}
      <div className="sticky top-0 w-full h-screen overflow-hidden flex justify-center pb-20">
        <div className="relative w-full max-w-[1400px] h-full mx-auto px-6">
          ...
          <motion.div 
            ...
            className="absolute inset-0 pt-16 flex flex-col items-center pointer-events-auto"
          >
            <div className="relative w-full">
              ...
              <div className="bg-white border rounded-[48px] p-2 relative z-10 w-full shadow-2xl">
                {/* Grid Content */}
                <div className="w-full bg-zinc-50/50 rounded-[40px] p-6 md:p-10 grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                  {/* 1. Project Progress / Current CO2 Card */}
                  <DashCard className="md:col-span-2 md:row-span-2 justify-between border-transparent shadow-[0_8px_30px_rgb(0,0,0,0.06)]">
                    <div>
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-10 mt-10">
                      {/* Semi-Circle Gauge */}
                      <div className="relative flex items-end justify-center w-80 h-40 overflow-hidden transform scale-110">
          ...
          >
            <div className="relative w-full">
              ...
              <div className="p-10 flex flex-col md:flex-row items-center justify-between gap-12 bg-[#18181b] rounded-[44px] overflow-hidden">
                ...
                <div className="flex-1 w-full flex flex-col items-end">
                  <span className="text-[70px] leading-none font-bold tracking-tighter text-emerald-400 mb-2">
                    <AnimatedNumber value={currentCredits} /> <span className="text-3xl text-emerald-600 font-semibold tracking-normal">/ 100</span>
                  </span>
          ...
          >
            <div className="relative w-full">
               ...
               <div className="p-8 flex flex-col md:flex-row gap-8 bg-[#18181b] rounded-[36px]">
                ...
                <div className="flex-1 bg-gradient-to-br from-[#111827] to-[#1e293b] rounded-3xl p-8 border border-white/5 relative overflow-hidden flex flex-col justify-between">
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-0 sm:-gap-4 relative z-10 my-4">
          ...
          >
            <div className="relative w-full">
              ...
              <div className="p-10 flex flex-col md:flex-row items-center gap-10 bg-zinc-900 rounded-[36px]">
